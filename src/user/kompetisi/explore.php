...
// AMBIL DATA KOMPETISI (DARI TABEL USERS/ADMIN)
// Kita anggap 1 Admin = 1 Event Organizer Utama
try {
...
<div class="p-6 sm:ml-64 pt-24 bg-slate-50 min-h-screen font-sans">
    ...
    <?php if(empty($events)): ?>
        <div class="flex flex-col items-center justify-center h-96 bg-white rounded-2xl border-2 border-dashed border-slate-300 text-center p-8">
            ...
            <h3 class="text-xl font-bold text-slate-700">Belum Ada Kompetisi Aktif</h3>
            <p class="text-slate-500 mt-2 text-xs">Admin/EO belum membuka pendaftaran untuk event baru.</p>
        </div>
    <?php else: ?>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php foreach($events as $e): 
                // Fallback Data (Mencegah Error jika Admin belum isi profil lengkap)
                $namaEvent = !empty($e['nama_lengkap']) ? $e['nama_lengkap'] : 'Event Tanpa Nama';
                $lokasi    = !empty($e['location']) ? $e['location'] : 'Indonesia';
                $tgl       = !empty($e['event_start_date']) ? date('d F Y', strtotime($e['event_start_date'])) : 'Segera';
                ...
                // Jika gambar lokal (bukan http), tambahkan path public
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl transition group duration-300 flex flex-col h-full hover:-translate-y-1">
                ...
                <div class="h-48 bg-slate-200 relative overflow-hidden">
                    <img src="<?= htmlspecialchars($img) ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-700" onerror="this.src='https://via.placeholder.com/500x300?text=No+Image'">
