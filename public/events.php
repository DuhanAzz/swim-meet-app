...
<body class="bg-slate-50 pt-32">
    ...
    <main class="max-w-screen-xl mx-auto px-6 py-12">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white p-8 rounded-[2.5rem] border border-slate-200 flex flex-col md:flex-row items-center gap-8 hover:border-blue-500 transition-all shadow-sm">
                <div class="w-24 h-24 bg-slate-100 rounded-3xl flex items-center justify-center text-4xl shrink-0 overflow-hidden">
                    <?php if($ev['profile_image']): ?> <img src="<?= $ev['profile_image'] ?>" class="w-full h-full object-cover"> <?php else: ?> 🏊 <?php endif; ?>
                </div>
                <div class="flex-1 text-center md:text-left">
                    <h3 class="text-2xl font-black uppercase text-slate-800 leading-tight"><?= $ev['nama_lengkap'] ?></h3>
                    <p class="text-slate-500 font-bold text-sm uppercase mt-2 tracking-widest">📅 <?= date('d M Y', strtotime($ev['event_start_date'])) ?> | 📍 <?= $ev['location'] ?></p>
                </div>
                <div class="flex gap-3 w-full md:w-auto min-w-[380px]">
                    <a href="<?= $pdf ?: '#' ?>" target="_blank" class="flex-1 text-center border-2 border-slate-200 py-4 px-6 rounded-2xl font-black text-[10px] uppercase hover:bg-slate-50 transition tracking-widest <?= !$pdf ? 'opacity-30' : '' ?>">📄 PDF Persyaratan</a>
                    <a href="login.php" class="flex-1 text-center bg-blue-600 text-white py-4 px-8 rounded-2xl font-black text-[10px] uppercase hover:bg-blue-700 shadow-lg tracking-widest">🎫 Daftar Sekarang</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
    ...
    <footer class="bg-[#0F172A] text-white pt-24 pb-12 mt-20 border-t-4 border-blue-600 text-center">
        <img src="img/logo.png" class="h-24 mx-auto mb-8 grayscale opacity-50">
        <p class="text-slate-500 text-[11px] font-black tracking-[0.5em] uppercase tracking-widest">&copy; 2025 SWIMMEET MANAGER</p>
    </footer>
</body>
</html>