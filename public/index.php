...
// Jika Maintenance AKTIF dan Pengunjung BUKAN Master -> Blokir Akses
    ...
    <body class="bg-slate-900 h-screen flex flex-col items-center justify-center text-center p-6 font-['Inter']">
        <div class="bg-slate-800 p-10 rounded-3xl shadow-2xl border border-slate-700 max-w-lg w-full">
            ...
            <div class="mt-8 pt-8 border-t border-slate-700">
                <a href="login.php" class="text-blue-500 hover:text-blue-400 text-xs font-bold uppercase tracking-widest hover:underline">
                    Login Admin
                </a>
            </div>
        </div>
...
<head>
    ...
    <style>
        ...
        #navbar { transition: all 0.4s; height: 110px; display: flex; align-items: center; }
...
<body class="bg-slate-50 text-slate-800">
    ...
    <nav id="navbar" class="fixed w-full z-50 top-0 start-0 transparent px-10">
        <div class="max-w-screen-2xl flex items-center justify-between mx-auto w-full">
            <a href="#home"><img src="img/logo.png" class="h-24 w-auto object-contain transition-all duration-300" id="nav-logo"></a>
            <div class="flex items-center gap-12">
                <div class="hidden lg:flex items-center space-x-10">
                    <a href="#home" class="nav-link font-bold uppercase text-white text-sm">Home</a>
                    <a href="events.php" class="nav-link font-bold uppercase text-white text-sm">Jadwal Lomba</a>
                    <a href="results.php" class="nav-link font-bold uppercase text-white text-sm">Hasil</a>
        ...
        <div class="max-w-screen-xl mx-auto px-6 w-full pt-48 relative z-10 text-white">
            <div class="max-w-5xl">
                ...
                <div class="mb-12 w-full md:w-3/4 bg-yellow-400 text-slate-900 px-1 py-1 rounded font-bold overflow-hidden shadow-2xl border-l-8 border-slate-900 transform -skew-x-6">
                    <div class="flex items-center bg-yellow-400 skew-x-6"><span class="bg-slate-900 text-yellow-400 text-xs px-4 py-2 uppercase font-black z-10">NEWS</span><div class="flex-1 overflow-hidden py-2"><marquee class="text-sm uppercase font-black" scrollamount="6"><?= htmlspecialchars($runningText) ?></marquee></div></div>
                </div>
                <?php endif; ?>
    ...
    <section id="schedule" class="py-32 px-6 max-w-screen-xl mx-auto section-scroll">
        <div class="flex justify-between items-end mb-16"><h2 class="text-5xl font-black uppercase italic text-slate-900 tracking-tighter">Competition Preview</h2><a href="events.php" class="text-blue-600 font-bold uppercase underline">Lihat Semua &rarr;</a></div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div class="bg-white rounded-[3rem] border border-slate-200 overflow-hidden hover:shadow-2xl transition group flex flex-col relative">
                ...
                <div class="p-10 flex-1"><h3 class="text-3xl font-black uppercase text-slate-800 mb-6 italic"><?= htmlspecialchars($e['event_name']) ?></h3><div class="flex gap-8 text-slate-500 text-xs font-bold uppercase"><span>📅 <?= date('d M Y', strtotime($e['event_date_start'])) ?></span><span>📍 <?= htmlspecialchars($e['event_location']) ?></span></div></div>
                <div class="grid grid-cols-2 border-t bg-slate-50"><a href="results.php?event_id=<?= $e['id'] ?>&cat=StartList" class="py-6 border-r flex flex-col items-center hover:bg-slate-900 hover:text-white transition uppercase text-[10px] font-black"><span>📖</span> Buku Acara</a><a href="results.php?event_id=<?= $e['id'] ?>&cat=Result" class="py-6 flex flex-col items-center hover:bg-blue-600 hover:text-white transition uppercase text-[10px] font-black"><span>🏆</span> Hasil</a></div>
            </div>
            <?php endforeach; ?>
        </div>
    </section>
        ...
        <div class="max-w-screen-xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 mb-16 text-center md:text-left">
            ...
            <div>
                ...
                <div class="flex justify-center md:justify-start gap-4">
                    ...
                    <a href="<?= htmlspecialchars($linkFB) ?>" target="_blank" class="group relative w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden transition hover:-translate-y-1 shadow-lg">
                        <div class="absolute inset-0 bg-[#1877F2] opacity-0 group-hover:opacity-100 transition"></div>
                        <svg class="w-5 h-5 text-white z-10" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clip-rule="evenodd" /></svg>
                    </a>
                </div>
            </div>
        </div>
